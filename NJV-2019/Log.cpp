...
namespace Log {
	...
	LOG getlog(wchar_t  logfile[]) {
		...
		if (!(*logFile.stream).is_open()) throw ERROR_THROW(105);
	...
	void writeIn(const LOG &log, const In::IN &in) {
		...
		*log.stream
			...
			<< "\nПроигнорировано    : " << std::setw(3) << in.ignor << endl;
	}
			...
			if (In.tokens[i].isLiteral) // ("Да") : ("Нет")) <<
			{
				...
				if (key == 0)
				{
					...
					if (In.tokens[i].token[0] == '"')
						lit.typel[k] = "word";
					else if ((In.tokens[i].token[0] == 'f' && In.tokens[i].token[1] == 'a' && In.tokens[i].token[2] == 'l' && In.tokens[i].token[3] == 's' && In.tokens[i].token[4] == 'e')||(In.tokens[i].token[0] == 't' && In.tokens[i].token[1] == 'r' && In.tokens[i].token[2] == 'u' && In.tokens[i].token[3] == 'e'))
						lit.typel[k] = "bool";
					else 
						lit.typel[k] = "byte";  
					k++;
		...
		*log.stream << "\nТаблица идентификаторов: \n" <<
		...
		for (int i = 0; i < IDtable.size; i++)
		{
			...
			*log.stream << std::setw(7) << std::left;
			...
			{
				if (IDtable.table[i].iddatatype == IT::STR)
					*log.stream << ((IDtable.table[i].value.vstr.len > 0) ? (IDtable.table[i].value.vstr.str) : ("/0"));
				else
				{
					if (IDtable.table[i].iddatatype == IT::BOOL)
						*log.stream << ((IDtable.table[i].value.vbool.len > 0) ? (IDtable.table[i].value.vbool.str) : ("false"));
					else *log.stream << "NULL";
				}
			}
			*log.stream << std::endl;
		}
