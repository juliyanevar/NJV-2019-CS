...
namespace MFST
{
	...
	struct Mfst				// магазинный автомат
	{
		...
		bool savestate(std::ofstream& filetrace);				// сохранить состояние автомата
		bool reststate(std::ofstream& filetrace);				// восстановить состояние автомата
		bool push_chain(				// поместить цепочку правила в стек
			GRB::Rule::Chain chain		// цепочка правила
		);
		RC_STEP step(std::ofstream& filetrace);					// выполнить шаг автомата
		bool start(std::ofstream& filetrace);					// запустить автомат
		bool savediagnosis(
